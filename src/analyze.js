...
const IMPORTANT = /\b(auth|security|database|migration|deploy|production|delete|payment|permission|architecture|schema|credential|secret)\b/i;
...
export function analyzeFiles(files, now = Date.now()) {
  ...
  for (const file of files) {
    ...
    let actions = 0, important = 0, reviews = 0, corrections = 0, users = 0;
    let lines;
    ...
    for (const line of lines) {
      ...
      if (isAction(row)) {
        actions += 1;
        if (IMPORTANT.test(JSON.stringify(row))) important += 1;
      }
    }
    total.sessions += 1;
    total.actions += actions;
    total.important += important;
    total.reviewed += Math.min(important, reviews);
    total.corrected += corrections;
    total.unreviewed += Math.max(0, important - reviews);
  }
